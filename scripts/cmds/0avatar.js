...
module.exports = {
  ...
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: 'تنزيل تلقائي للفيديو لـ إنستغرام. أو فيسبوك أو تيكتوك',
    longDescription: '',
    category: 'وسائط',
    guide: {
      en: '{p}{n}',
  ...
  onStart: async function ({ api, event }) {
    ...
    if (event.body.toLowerCase().includes('تلقائي')) {
      api.sendMessage(" ✅ | التحميل التلقائي للفيديوهات نشط.", event.threadID, event.messageID);
    }
  },
    ...
    this.getLink(url, api, event, path).then(res => {
      axios({
        ...
        if (fs.statSync(path).size / 1024 / 1024 > 25) {
          return api.sendMessage(" ❌ | للفيديو كبير من ناحية الحجم لإرساله ", event.threadID, () => fs.unlinkSync(path), event.messageID);
        }
        api.sendMessage({
      ...
      if (res.success && res.download && res.download.length > 0) {
        ...
        axios({
          ...
          if (res.headers['content-length'] > 87031808) {
            return api.sendMessage(" ⚠️ |الفيديو كبير من ناحية الحجم لإرساله", event.threadID, () => fs.unlinkSync(path), event.messageID);
          }
          res.data.pipe(fs.createWriteStream(path));
          res.data.on('end', () => {
            api.sendMessage({
              body: " ✅ | تم تنزيل الفيدو بنجاح",
              attachment: fs.createReadStream(path)
            }, event.threadID, () => fs.unlinkSync(path), event.messageID);
          });
        }).catch(err => console.error(err));
      } else {
        api.sendMessage(" ❌ |فشل في تنزيل فيديو الفيسبوك", event.threadID, event.messageID);
      }
    }).catch(err => console.error(err));
  },
