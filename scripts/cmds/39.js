...
module.exports = {
    ...
    onStart: async function ({ message, event, args }) {
        ...
        if (mention.length == 1) {
            const id = mention[0]
            bal(id).then(ptth => { message.reply({ body: "أنظر إلى هذا الشخص الغامض", attachment: fs.createReadStream(ptth) }) })
        } else {
            const men = event.senderID
            bal(men).then(ptth => { message.reply({ body: "أنظر إلى هذا الشخص الغامض", attachment: fs.createReadStream(ptth) }) })
        }
    }
