module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    let text = args.join(" ")
    if (!text) return api.sendMessage('💢الرجاء إدخال التنسيق الصحيح [نص1 - نص2] ', event.threadID, event.messageID);
    const text1 = text.substr(0, text.indexOf(' - '));
    if (!text1) return api.sendMessage('💢الرجاء إدخال التنسيق الصحيح [نص1 - نص2] ', event.threadID, event.messageID);
    const text2 = text.split(" - ").pop()
    if (!text2) return api.sendMessage('💢الرجاء إدخال الصيغة الصحيحة [نص1 - نص2] ', event.threadID, event.messageID);
